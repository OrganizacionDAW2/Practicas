...
public class Cabeceras extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<link rel=\"stylesheet\" type=\"text/css\" href=\"" + request.getContextPath() + "/CSS/Cabecera.css\" media=\"screen\" />");
            out.println("</head>");
            out.println("<body>");
            out.println("<div>");
            out.println("<h1>Cabeceras de registro.</h1>");
            
            Enumeration<String> cabeceras = request.getHeaderNames();
            while (cabeceras.hasMoreElements()) {
                String nombre = cabeceras.nextElement();
                out.println("<strong>" + nombre + "</strong> - " + request.getHeader(nombre) + "<br/>");
            }
